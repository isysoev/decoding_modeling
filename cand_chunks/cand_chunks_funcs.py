...
def candidate_chunks(data_df):
    ...
    cand_chunks = {'pre': CandChunks(), 'post': CandChunks(), 'entire': CandChunks()}
    for i in range(len(data_df)):
        _update_word_candidate_chunks(data_df.iloc[i], cand_chunks)
    
    
    #Selects the top pronunication to use.
    top_ipa_chunks = {cand_type: cand_chunks[cand_type].give_argmax_pronunications()\
                    for cand_type in ['pre', 'post']} #Do not filter for the words.
